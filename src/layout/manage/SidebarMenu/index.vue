...
import mangeRoute from '@/router/modules/manage.ts';
import { computed } from 'vue';

// 遍历子路由并打印信息
if (mangeRoute.children) {
  mangeRoute.children.forEach((route: any) => {
...
const menuRoutes = computed<RouteRecordRaw[]>(() => {
  const children = mangeRoute.children;
  if (!children) return [];
  return children.map((childRoute) => ({
    ...childRoute,
    path: `${mangeRoute.path}/${childRoute.path}`
  }));
});
